document.addEventListener("DOMContentLoaded", () => {
	...
	const fetchAndRenderPosts = async () => {
		...
		if (!postsApiUrl) {
			statusNode.textContent = "Start a local PHP server (for example: php -S localhost:8000), then open this page through http://localhost:8000.";
			feedNode.innerHTML = "";
			return;
		}
		...
		try {
			...
			statusNode.textContent = "Could not load posts right now. Make sure the PHP server is running and reachable.";
			feedNode.innerHTML = "";
		}
	};
